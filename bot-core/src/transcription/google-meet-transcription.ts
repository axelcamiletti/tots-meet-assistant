...
export class GoogleMeetTranscription {
  private page: Page;
  private transcriptionEntries: TranscriptionEntry[] = [];
  private isRecording: boolean = false;
  private pollingInterval: NodeJS.Timeout | null = null;
  private captionMonitorInterval: NodeJS.Timeout | null = null;
  private options: TranscriptionOptions;

  constructor(page: Page, options: TranscriptionOptions = {}) {
    this.page = page;
    this.options = {
  ...
  async startTranscription(): Promise<void> {
    ...
    try {
      ...
      await this.startCaptionMonitor();
  ...
  private async enableLiveCaptions(): Promise<void> {
    ...
    try {
      ...
      for (const selector of captionSelectors) {
        try {
          captionButton = await this.page.waitForSelector(selector, { timeout: 3000 });
          if (captionButton) {
      ...
      if (captionButton) {
        ...
        await this.configureCaptionLanguage();
      } else {
   ...
   */
  private async configureCaptionLanguage(): Promise<void> {
    try {
      ...
      for (const selector of languageComboBoxSelectors) {
        try {
          const languageButton = await this.page.waitForSelector(selector, { timeout: 3000 });
          if (languageButton) {
            ...
            await this.selectSpanishFromDropdown();
            return;
          }
        } catch (e) {
  ...
  private async startPolling(): Promise<void> {
    ...
    this.pollingInterval = setInterval(async () => {
      if (this.isRecording && !this.page.isClosed()) {
        await this.captureTranscriptions();
      }
   ...
   */
  private async startCaptionMonitor(): Promise<void> {
    console.log('👁️ [TRANSCRIPTION] Iniciando monitor persistente de subtítulos...');
    
    this.captionMonitorInterval = setInterval(async () => {
      if (this.isRecording && !this.page.isClosed()) {
        await this.checkAndReactivateCaptions();
      }
   ...
   */
  private async checkAndReactivateCaptions(): Promise<void> {
    try {
      console.log('🔍 [TRANSCRIPTION] Verificando estado de subtítulos...');
      
      // Buscar indicadores de que los subtítulos están desactivados
      const captionsOff = await this.page.$('[aria-label*="Turn on captions"], [aria-label*="Activar subtítulos"]');
      ...
      if (captionsOff) {
        ...
        await this.checkCurrentLanguage();
      }
    } catch (error) {
   ...
   */
  private async checkCurrentLanguage(): Promise<void> {
    try {
      // Buscar el combobox de idioma para ver qué idioma está seleccionado
      const languageCombobox = await this.page.$('[aria-label="Meeting language"]');
      if (languageCombobox) {
        const currentLanguage = await languageCombobox.textContent();
        console.log(`🌐 [TRANSCRIPTION] Idioma actual: "${currentLanguage?.trim()}"`);
   ...
   */
  private async selectSpanishFromDropdown(): Promise<void> {
    try {
      ...
      try {
        const allOptions = await this.page.$$eval('li[role="option"], [role="option"]', options => 
          options.map(option => ({
      ...
      for (const selector of spanishSelectors) {
        try {
          ...
          if (element) {
            ...
            console.log(`✅ [TRANSCRIPTION] Opción de español encontrada: "${text?.trim()}"`);
      ...
      try {
        const allOptions = await this.page.$$('li[role="option"], [role="option"]');
   ...
   */
  private async captureTranscriptions(): Promise<void> {
    try {
    ...
    const transcriptionSelectors = [
      ...
      for (const selector of transcriptionSelectors) {
        try {
          const captionElements = await this.page.$$(selector);
          ...
          for (const element of captionElements) {
            ...
            if (text && text.trim().length > 0) {
              ...
              if (this.isLikelyTranscription(text.trim())) {
                await this.processCaptionText(text.trim());
                captionsFound = true;
   ...
   */
  private async processCaptionText(text: string): Promise<void> {
   ...
   */
  private cleanUIElements(text: string): string {
    let cleaned = text;
   ...
   */
  getTranscriptionsSince(since: Date): TranscriptionEntry[] {
